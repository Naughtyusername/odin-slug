...
main :: proc() {
	...
	for running {
		...
		// then the clipped content in a separate flush below.
		slug.draw_rect(
			ctx,
			CLIP_BOX_X,
			CLIP_BOX_Y,
		...
		slug.draw_rect_bordered(ctx, UI_PANEL_X, UI_PANEL_Y, UI_PANEL_W, UI_PANEL_H,
			{0.1, 0.1, 0.18, 1.0}, {0.4, 0.4, 0.7, 1.0}, border = 2)
		slug.draw_text(ctx, "Bordered Panel", UI_PANEL_X + 15, UI_PANEL_Y + 26, SMALL_SIZE, COLOR_WHITE)

		hp := 72.0 + math.sin(elapsed * 0.5) * 28.0
		slug.draw_bar(ctx, BAR_X, BAR_Y, BAR_W, BAR_H, f32(hp), 100,
			{0.2, 0.8, 0.3, 1.0}, {0.15, 0.15, 0.25, 1.0},
			label = fmt.tprintf("HP %d/100", int(hp)), label_size = 14,
			label_color = COLOR_WHITE, border_color = {0.4, 0.6, 0.4, 1.0}, border = 1)
		mp := 35.0 + math.sin(elapsed * 0.7) * 15.0
		slug.draw_bar(ctx, BAR_X, BAR_Y + 30, BAR_W, BAR_H, f32(mp), 80,
			{0.3, 0.4, 0.9, 1.0}, {0.15, 0.15, 0.25, 1.0},
			label = fmt.tprintf("MP %d/80", int(mp)), label_size = 14,
			border_color = {0.3, 0.4, 0.7, 1.0}, border = 1)
		...
		slug.draw_rect_bordered(ctx, RICH_WRAP_X, RICH_WRAP_Y, RICH_WRAP_W, 120,
			{0.08, 0.08, 0.14, 1.0}, {0.3, 0.3, 0.5, 1.0}, border = 1)
		slug.draw_rich_text_wrapped(ctx,
			...
			RICH_WRAP_X + 10, RICH_WRAP_Y + 8, SMALL_SIZE, RICH_WRAP_W - 20,
			COLOR_WHITE, line_spacing = 1.4)
		...
		// --- Present ---
		if !slug_vk.present_frame(renderer) {
			fmt.eprintln("Present frame failed")
			break
		}
	}
