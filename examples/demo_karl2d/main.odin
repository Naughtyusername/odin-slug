...
// ===================================================
// odin-slug Karl2D Demo
//
// Standalone example showing GPU-accelerated Bezier text rendering
// using odin-slug with the Karl2D backend. Uses Karl2D for windowing,
// input, and the OpenGL 3.3 context. Showcases all library features:
...
//   - Karl2D source (https://github.com/karl-zylinski/karl2d)
// ===================================================
...
WINDOW_TITLE :: "odin-slug Karl2D Demo"
WINDOW_WIDTH :: 1600
WINDOW_HEIGHT :: 1060
...
FLOAT_X :: f32(310) // floating damage number
FLOAT_Y :: f32(426)
...
main :: proc() {
	// -----------------------------------------------
	// 1. Initialize Karl2D and create window
	// -----------------------------------------------

	_ = k2.init(WINDOW_WIDTH, WINDOW_HEIGHT, WINDOW_TITLE, {window_mode = .Windowed_Resizable})
	defer k2.shutdown()
	...
	defer slug_k2.destroy(renderer)
...

	{
		...
		slug_k2.upload_shared_textures(renderer, &pack)
	}
	...
	for k2.update() {
		...
		width := i32(k2.get_screen_width())
		height := i32(k2.get_screen_height())
		...
		// Cursor keys
		if k2.key_went_down(.Left) && cursor_idx > 0 do cursor_idx -= 1
		if k2.key_went_down(.Right) && cursor_idx < len(cursor_text) do cursor_idx += 1

		// Zoom
		if k2.key_went_down(.Up) do slug.set_ui_scale(ctx, clamp(ctx.ui_scale + 0.25, ZOOM_MIN, ZOOM_MAX))
		if k2.key_went_down(.Down) do slug.set_ui_scale(ctx, clamp(ctx.ui_scale - 0.25, ZOOM_MIN, ZOOM_MAX))
		if k2.key_went_down(.Tab) do slug.set_ui_scale(ctx, ZOOM_FIT_SCALE if ctx.ui_scale != ZOOM_FIT_SCALE else 1.0)

		// WASD camera pan
		if k2.key_is_held(.W) do cam_y -= CAMERA_SPEED * dt
		if k2.key_is_held(.A) do cam_x -= CAMERA_SPEED * dt
		if k2.key_is_held(.S) do cam_y += CAMERA_SPEED * dt
		if k2.key_is_held(.D) do cam_x += CAMERA_SPEED * dt
		if k2.key_went_down(.R) { cam_x = 0; cam_y = 0 }

		// Middle mouse drag
		if k2.mouse_button_went_down(.Middle) do mid_drag = true
		if k2.mouse_button_went_up(.Middle) do mid_drag = false
		if mid_drag {
		...
		// Mouse wheel: scroll region or zoom
		wheel := k2.get_mouse_wheel_delta()
		if wheel != 0 {
			mouse_pos := k2.get_mouse_position()
			wmx := mouse_pos.x - cam_x
			wmy := mouse_pos.y - cam_y
		...
		slug.draw_text_shadow(
			ctx, "Slug + Karl2D", LEFT_X, ROW_TITLE,
			slug.scaled_size(ctx, TITLE_SIZE), COLOR_WHITE, shadow_offset = 2.0,
		)

		// Subtitle with outline
		slug.draw_text_outlined(
			ctx, "GPU Bezier text — Karl2D backend.", LEFT_X, ROW_SUBTITLE,
			slug.scaled_size(ctx, BODY_SIZE), COLOR_CYAN,
			outline_thickness = 2.5, outline_color = {0.8, 0.2, 0.8, 1.0},
		)

		// Rich text markup
		slug.draw_rich_text(
			ctx, "You deal {red:15} dmg with {icon:128:yellow}{yellow:Golden Sword}!",
			LEFT_X, ROW_RICH_TEXT, BODY_SIZE, COLOR_WHITE,
		)
		...
		slug.draw_rich_text(
			ctx, "Status: {bg:red:POISONED}  {bg:green:HASTE}  {bg:#884400:BURNING}",
			LEFT_X, ROW_STATUS, SMALL_SIZE, COLOR_WHITE,
		)
		...
		slug.draw_text(
			ctx, fmt.tprintf("[</>] or click  idx:%d", cursor_idx),
			LEFT_X, ROW_CURSOR_HINT, 14, {0.5, 0.5, 0.5, 1.0},
		)
		...
		{
			fx_font := slug.active_font(ctx)
			px := FX_X

			hw, _ := slug.measure_text(fx_font, "H", SMALL_SIZE)
			slug.draw_text(ctx, "H", px, FX_SUBSUP_Y, SMALL_SIZE, COLOR_WHITE)
			px += hw

			sub2w, _ := slug.measure_text(fx_font, "2", SMALL_SIZE * slug.SUB_SCALE)
			slug.draw_text_sub(ctx, "2", px, FX_SUBSUP_Y, SMALL_SIZE, {0.5, 0.85, 1.0, 1.0})
			px += sub2w

			ow, _ := slug.measure_text(fx_font, "O", SMALL_SIZE)
			slug.draw_text(ctx, "O", px, FX_SUBSUP_Y, SMALL_SIZE, COLOR_WHITE)
			px += ow + 35

			xw, _ := slug.measure_text(fx_font, "x", SMALL_SIZE)
		...
		// Circular orbit + rotated text
		slug.draw_text_on_circle(
			ctx, "  text orbiting a circle  ", CIRCLE_CX, CIRCLE_CY, CIRCLE_R + 20,
			start_angle = -elapsed * 0.4, font_size = SMALL_SIZE, color = {0.8, 0.5, 1.0, 1.0},
		)
		...
		slug.draw_text_truncated(
			ctx, "This long name gets clipped with an ellipsis",
			RIGHT_X + 10, TRUNCATE_Y, SMALL_SIZE, TRUNCATE_MAX_W, COLOR_WHITE,
			ellipsis = " [...]",
		)
		...
		slug.draw_text(
			ctx, fmt.tprintf("cell: %.0fx%.0fpx", grid_cell_w, grid_cell_h),
			RIGHT_X, GRID_Y + grid_cell_h * 2 + 4, 13, {0.5, 0.5, 0.7, 1.0},
		)
		...
		slug.draw_text_justified(
			ctx, "Word justification fills the column width exactly.",
			RIGHT_X, JUSTIFY_Y, SMALL_SIZE, JUSTIFY_W, {0.9, 0.8, 0.6, 1.0},
		)
		...
		// Scissor demo: draw the box outline and label (unclipped) in this pass
		...
		// Scale indicator
		slug.draw_text(
			ctx, fmt.tprintf("Scale: %.2fx [Up/Down/Wheel/Tab]  Cam: %.0f,%.0f [WASD/MMB  R=reset]", ctx.ui_scale, cam_x, cam_y),
			10, SCALE_Y, 16, {0.5, 0.5, 0.5, 1.0},
		)
	...
	fmt.println("Karl2D demo exiting.")
}
