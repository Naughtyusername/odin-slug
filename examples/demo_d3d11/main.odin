...
// ===================================================
// odin-slug D3D11 Demo
//
// Standalone example showing GPU-accelerated Bezier text rendering
// using odin-slug with the Direct3D 11 backend. Uses raw Win32 for
// windowing and D3D11 for rendering. Showcases all library features:
// effects, wrapping, scrolling, cursor positioning, rich text,
// caching, alignment, shared font atlas, etc.
//
// Build:  odin build examples/demo_d3d11/ -out:demo_d3d11.exe -collection:libs=.
// Run:    demo_d3d11.exe
//
// Prerequisites:
//   - Liberation fonts in examples/assets/fonts/
//   - Windows with D3D11-capable GPU
// ===================================================
...
WINDOW_TITLE :: "odin-slug D3D11 Demo"
WINDOW_WIDTH :: 1600
WINDOW_HEIGHT :: 1060
...
// ===================================================
// Global state for window proc callbacks
// ===================================================

g_scroll_accum: f32 = 0
g_mouse_x: f32 = 0
g_mouse_y: f32 = 0
g_mouse_down: bool = false
g_mouse_pressed: bool = false // true for one frame on click
g_running: bool = true
g_width: i32 = WINDOW_WIDTH
g_height: i32 = WINDOW_HEIGHT
g_resized: bool = false

wnd_proc :: proc "system" (hwnd: win32.HWND, msg: win32.UINT, wp: win32.WPARAM, lp: win32.LPARAM) -> win32.LRESULT {
	switch msg {
	case win32.WM_DESTROY:
		...
		return 0
	case win32.WM_SIZE:
		w := i32(win32.LOWORD(u32(lp)))
		h := i32(win32.HIWORD(u32(lp)))
		if w > 0 && h > 0 {
			g_width = w
			g_height = h
			g_resized = true
	...
	case win32.WM_MOUSEWHEEL:
		delta := i16(win32.HIWORD(u32(wp)))
		g_scroll_accum += f32(delta) / 120.0
		return 0
	case win32.WM_MOUSEMOVE:
		g_mouse_x = f32(i16(win32.LOWORD(u32(lp))))
		g_mouse_y = f32(i16(win32.HIWORD(u32(lp))))
		return 0
...
key_down :: proc(vk: i32) -> bool {
	return (win32.GetAsyncKeyState(vk) & transmute(i16)u16(0x8000)) != 0
}
...
main :: proc() {
	...
	class_name := win32.L("slug_d3d11_demo")
	h_instance := win32.HINSTANCE(win32.GetModuleHandleW(nil))
	wc := win32.WNDCLASSEXW{
		cbSize = size_of(win32.WNDCLASSEXW),
		style = win32.CS_HREDRAW | win32.CS_VREDRAW,
		lpfnWndProc = wnd_proc,
		hInstance = h_instance,
		hCursor = win32.LoadCursorW(nil, win32.LPCWSTR(win32.MAKEINTRESOURCEW(32512))), // IDC_ARROW
		lpszClassName = win32.LPCWSTR(class_name),
	}
	win32.RegisterClassExW(&wc)

	// Adjust window rect so client area matches desired size
	rect := win32.RECT{right = WINDOW_WIDTH, bottom = WINDOW_HEIGHT}
	style :: win32.WS_OVERLAPPEDWINDOW
	win32.AdjustWindowRect(&rect, style, false)

	hwnd := win32.CreateWindowExW(
		0,
		win32.LPCWSTR(class_name),
	...
	if hr < 0 {
		fmt.eprintln("Failed to create D3D11 device")
		return
	}
	defer {
		dc->Release()
	...
	create_rtv :: proc(device: ^d3d11.IDevice, swapchain: ^dxgi.ISwapChain) -> ^d3d11.IRenderTargetView {
		bb: ^d3d11.ITexture2D
		swapchain->GetBuffer(0, d3d11.ITexture2D_UUID, (^rawptr)(&bb))
		if bb == nil do return nil
		defer bb->Release()
		result: ^d3d11.IRenderTargetView
		device->CreateRenderTargetView(bb, nil, &result)
		return result
	}
	rtv = create_rtv(device, swapchain)
	if rtv == nil {
		fmt.eprintln("Failed to create render target view")
		return
	}
	...
	defer slug_d3d.destroy(renderer)

	ctx := slug_d3d.ctx(renderer)

	// -----------------------------------------------
	// 4. Load fonts + SVG icons into shared atlas
	// -----------------------------------------------
	...
	for g_running {
		...
		if g_resized {
			g_resized = false
			if rtv != nil do rtv->Release()
			swapchain->ResizeBuffers(0, u32(g_width), u32(g_height), .UNKNOWN, {})
			rtv = create_rtv(device, swapchain)
		}
		...
		// Input
		if key_down(win32.VK_UP)   do slug.set_ui_scale(ctx, clamp(ctx.ui_scale + 0.01, ZOOM_MIN, ZOOM_MAX))
		if key_down(win32.VK_DOWN) do slug.set_ui_scale(ctx, clamp(ctx.ui_scale - 0.01, ZOOM_MIN, ZOOM_MAX))
		tab_down := key_down(win32.VK_TAB)
		if tab_down && !prev_tab_down {
			slug.set_ui_scale(ctx, ZOOM_FIT_SCALE if ctx.ui_scale != ZOOM_FIT_SCALE else 1.0)
		}
		prev_tab_down = tab_down

		if key_down(0x57) do cam_y -= CAMERA_SPEED * dt // W
		if key_down(0x53) do cam_y += CAMERA_SPEED * dt // S
		if key_down(0x41) do cam_x -= CAMERA_SPEED * dt // A
		if key_down(0x44) do cam_x += CAMERA_SPEED * dt // D
		if key_down(0x52) { cam_x = 0; cam_y = 0 }     // R

		if key_down(win32.VK_LEFT)  && cursor_idx > 0              do cursor_idx -= 1
		if key_down(win32.VK_RIGHT) && cursor_idx < len(cursor_text) do cursor_idx += 1
		...
		if g_mouse_pressed {
			...
			if idx, hit := slug.text_hit_test(
				cursor_font, cursor_text,
				LEFT_X, ROW_CURSOR, SMALL_SIZE,
				g_mouse_x - cam_x, g_mouse_y - cam_y,
			); hit {
				cursor_idx = idx
			}
		}
		...
		// Scroll
		scroll_content_h, _ := slug.measure_text_wrapped(ctx, SCROLL_TEXT, SMALL_SIZE, scroll_region.width)
		world_mx := g_mouse_x - cam_x
		world_my := g_mouse_y - cam_y
		if g_scroll_accum != 0 {
			if world_mx >= scroll_region.x &&
			   ...
			   world_my <= scroll_region.y + scroll_region.height {
				slug.scroll_by(&scroll_region, -g_scroll_accum * 20.0, scroll_content_h)
			} else {
				slug.set_ui_scale(ctx, clamp(ctx.ui_scale + g_scroll_accum * ZOOM_WHEEL_STEP, ZOOM_MIN, ZOOM_MAX))
			}
			g_scroll_accum = 0
		}
		...
		slug.draw_text_shadow(ctx, "Slug + D3D11", LEFT_X, ROW_TITLE, slug.scaled_size(ctx, TITLE_SIZE), COLOR_WHITE, shadow_offset = 2.0)
		slug.draw_text_outlined(ctx, "GPU Bezier text \u2014 Direct3D 11 backend.", LEFT_X, ROW_SUBTITLE, slug.scaled_size(ctx, BODY_SIZE), COLOR_CYAN, outline_thickness = 2.5, outline_color = {0.8, 0.2, 0.8, 1.0})
		...
		slug.draw_text_truncated_word(ctx, "Word-boundary truncation clips at spaces", RIGHT_X + 10, TRUNCATE_WORD_Y, SMALL_SIZE, TRUNCATE_MAX_W, {0.8, 0.8, 0.6, 1.0})
		...
		slug.draw_text_selection(ctx, "Select range demo", RIGHT_X, SELECTION_Y, SMALL_SIZE, COLOR_WHITE, 7, 12, {0.2, 0.3, 0.8, 0.6})
		...
		slug.draw_rect_bordered(ctx, UI_PANEL_X, UI_PANEL_Y, UI_PANEL_W, UI_PANEL_H, {0.1, 0.1, 0.18, 1.0}, {0.4, 0.4, 0.7, 1.0}, border = 2)
		...
		slug.draw_bar(ctx, BAR_X, BAR_Y, BAR_W, BAR_H, hp, 100, {0.2, 0.8, 0.3, 1.0}, {0.15, 0.15, 0.25, 1.0}, label = fmt.tprintf("HP %d/100", int(hp)), label_size = 14, label_color = COLOR_WHITE, border_color = {0.4, 0.6, 0.4, 1.0}, border = 1)
		mp := f32(35.0 + math.sin(elapsed * 0.7) * 15.0)
		slug.draw_bar(ctx, BAR_X, BAR_Y + 30, BAR_W, BAR_H, mp, 80, {0.3, 0.4, 0.9, 1.0}, {0.15, 0.15, 0.25, 1.0}, label = fmt.tprintf("MP %d/80", int(mp)), label_size = 14, border_color = {0.3, 0.4, 0.7, 1.0}, border = 1)
		...
		slug.draw_rect_bordered(ctx, RICH_WRAP_X, RICH_WRAP_Y, RICH_WRAP_W, 120, {0.08, 0.08, 0.14, 1.0}, {0.3, 0.3, 0.5, 1.0}, border = 1)
		slug.draw_rich_text_wrapped(ctx, "The {red:goblin} attacks for {yellow:8 damage}! You counter with {icon:128:yellow}{yellow:Golden Sword} for {green:15 damage}. The {red:goblin} is {bg:red:DEFEATED}! You gain {cyan:25 XP}.", RICH_WRAP_X + 10, RICH_WRAP_Y + 8, SMALL_SIZE, RICH_WRAP_W - 20, COLOR_WHITE, line_spacing = 1.4)

		slug.draw_text(ctx, fmt.tprintf("Scale: %.2fx [Up/Down/Wheel/Tab]  Cam: %.0f,%.0f [WASD  R=reset]", ctx.ui_scale, cam_x, cam_y), 10, SCALE_Y, 16, {0.5, 0.5, 0.5, 1.0})

		slug.end(ctx)
		slug_d3d.flush(renderer, fb_w, fb_h, rtv)

		// Pass 2: clipped panel text
		slug.begin(ctx)
		slug.use_font(ctx, 0)
		slug.set_camera(ctx, cam_x, cam_y)
		slug.draw_text(ctx, "GPU-clipped panel text overflows \u2192", CLIP_BOX_X + 5, CLIP_TEXT_Y, SMALL_SIZE, COLOR_WHITE)
		slug.end(ctx)
		slug_d3d.flush(renderer, fb_w, fb_h, rtv, scissor = slug.Scissor_Rect{
			x = CLIP_BOX_X + cam_x,
			y = CLIP_BOX_Y + cam_y,
			w = CLIP_BOX_W,
			h = CLIP_BOX_H,
