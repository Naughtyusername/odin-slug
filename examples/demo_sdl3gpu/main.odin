...
// ===================================================
// odin-slug SDL3 GPU Demo
//
// Standalone example showing GPU-accelerated Bezier text rendering
// using odin-slug with the SDL3 GPU backend. Uses SDL3 for windowing
// and SDL3 GPU for cross-platform graphics (Vulkan/D3D12/Metal).
...
//   2. Build demo:      odin build examples/demo_sdl3gpu/ -collection:libs=.
// Run:    ./demo_sdl3gpu
//
// Prerequisites:
...
import slug "../../slug"
import slug_sdl3 "../../slug/backends/sdl3gpu"

// --- Window ---

WINDOW_TITLE :: "odin-slug SDL3 GPU Demo"
WINDOW_WIDTH :: 1600
WINDOW_HEIGHT :: 900
...
// Circle (orbital text + rotated text)
CIRCLE_CX :: f32(560)
CIRCLE_CY :: f32(490)
CIRCLE_R :: f32(90)
...
main :: proc() {
	// -----------------------------------------------
	// 1. Initialize SDL3 and create window
	// -----------------------------------------------
	...
	window := sdl.CreateWindow(WINDOW_TITLE, WINDOW_WIDTH, WINDOW_HEIGHT, {.RESIZABLE})
	if window == nil {
	...
	// -----------------------------------------------
	// 2. Create GPU device and initialize renderer
	// -----------------------------------------------
	...
	for running {
		...
		// --- Begin frame ---
		if !slug_sdl3.begin_frame(renderer) do continue
		...
		slug.draw_text_outlined(
			ctx,
			"GPU Bezier text — SDL3 GPU backend.",
			LEFT_X,
			ROW_SUBTITLE,
		...
		// --- Present ---
		if !slug_sdl3.present_frame(renderer) {
			fmt.eprintln("Present frame failed")
			break
		}
	}

	fmt.println("SDL3 GPU demo exiting.")
}
